...
import { ProgressInsight } from "@/components/account/progress-insight";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: t.account.childMeta };
}
...
export default async function ChildDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [child, { t, f }, assistant] = await Promise.all([getChildDetail(id, user.id), getI18n(), getAssistantSettings()]);
  if (!child) notFound();
  const a = t.account;
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        ...
        >
          <ArrowLeft aria-hidden className="h-4 w-4" /> {a.allChildren}
        </Link>

        <div className="mt-4 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <ChildAvatar firstName={child.firstName} lastName={child.lastName} hue={child.avatarHue} size="lg" />
            <div>
        ...
        {child.notes && <p className="mt-4 rounded-xl bg-surface-sunken p-4 text-sm text-ink-soft">{child.notes}</p>}
      </div>
      ...
      <dl className="grid gap-4 sm:grid-cols-3">
        ...
        <StatCard
          icon={Target}
          label={a.averageScore}
          value={child.averagePercent != null ? `${child.averagePercent}%` : "—"}
          hint={f.count(child.exams.length, t.units.exam)}
        />
        <StatCard icon={Award} label={a.bestResult} value={child.bestPercent != null ? `${child.bestPercent}%` : "—"} />
      </dl>

      {assistant.enabled && (
        <ProgressInsight childId={child.id} name={child.firstName} hasExams={child.exams.length > 0} />
      )}

      <section className="rounded-2xl border border-border bg-surface p-6 shadow-glow-sm">
        <h2 className="font-display text-lg font-semibold text-ink">{a.progressTitle}</h2>
        <p className="mt-1 text-sm text-muted">{a.progressText}</p>
        <div className="mt-6">
          {child.exams.length > 1 ? (
            ...
            <p className="rounded-xl bg-surface-sunken p-6 text-sm text-muted">
              {child.exams.length === 1 ? a.oneResult : a.noResults}
            </p>
          )}
        </div>
              ...
              <li key={exam.id} className="flex items-center justify-between gap-4 py-3">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted">
                    {f.date(exam.examDate)}
                    {exam.courseTitle ? ` · ${exam.courseTitle}` : ""}
                  </p>
                </div>
                <div className="shrink-0 text-right">
                  <p className="font-display text-base font-bold text-ink">{exam.percent}%</p>
                  <p className="text-xs text-muted">{tpl(t.format.scoreOf, { score: exam.score, max: exam.maxScore })}</p>
                </div>
              </li>
            ))}
          </ul>
      ...
      <section>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="font-display text-lg font-semibold text-ink">{a.courses}</h2>
          <Button asChild variant="outline" size="sm">
            <Link href="/courses">{a.enrollOnCourse}</Link>
          </Button>
        </div>

        {child.enrollments.length === 0 ? (
          <p className="mt-4 rounded-2xl border border-dashed border-border p-5 text-sm text-muted">
            {tpl(a.notEnrolled, { name: child.firstName })}
          </p>
        ) : (
          <ul className="mt-4 flex flex-col gap-2">
            {child.enrollments.map((enrollment) => (
              <li key={enrollment.id}>
                ...
                >
                  <div className="min-w-0">
                    <p className="truncate text-sm font-semibold text-ink">{enrollment.courseTitle}</p>
